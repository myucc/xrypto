...
#define htobe32(value) SWAPINT32((uint32) (value))
...
void sha256_finish(sha256_context* context, uint8* digest)
{
    ...
    context->w[14] = htobe32((uint32)(totalSize >> 32));
    context->w[15] = htobe32((uint32)totalSize);

    sha256_process(context);
    for (i = 0; i < 8; i++) context->h[i] = htobe32(context->h[i]);
    if (digest != 0) zmemcpy(digest, context->digest, 32);
}