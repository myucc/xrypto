...
unsigned int BlowFish::F(unsigned int x) {
    ...
    unsigned int y;
    d = x & 255; x >>= 8; d = x & 255; x >>= 8; c = x & 255; x >>= 8; b = x & 255; x >>= 8; a = x & 255;
    y = X[0][a] + X[1][b]; y ^= X[2][c]; y += X[3][d];
    return y;
}

void BlowFish::decrypt(unsigned int& xl, unsigned int& xr)
{
    unsigned int Xl = xl, Xr = xr, i;
    for (i = 17; i > 1; --i) { Xl ^= Z[i]; Xr = F(Xl) ^ Xr; G(Xl, Xr); }
    G(Xl, Xr); Xr ^= Z[1]; Xl ^= Z[0]; xl = Xl; xr = Xr;
}

void BlowFish::encrypt(unsigned int& xl, unsigned int& xr)
{
    unsigned int Xl = xl, Xr = xr, i;
    for (i = 0; i < 16; ++i) { Xl ^= Z[i]; Xr = F(Xl) ^ Xr; G(Xl, Xr); }
    G(Xl, Xr); Xr ^= Z[16]; Xl ^= Z[17]; xl = Xl; xr = Xr;
}

BlowFish::BlowFish(const char* key, unsigned long long length)
{
    unsigned int v, l{}, r{}, i, k, j{}, c = length;
    for (i = 0; i < 4; i++)for (k = 0; k < 256; k++) X[i][k] = S[i][k];
    for (i = 0; i < 18; ++i) { v = 0; for (k = 0; k < 4; ++k) { v = (v << 8) | key[j]; if (++j >= c) j = 0; }Z[i] = P[i] ^ v; }
    for (i = 0; i < 18; i += 2) { encrypt(l, r); Z[i] = l; Z[i + 1] = r; }
    for (i = 0; i < 4; ++i) for (k = 0; k < 256; k += 2) { encrypt(l, r); X[i][j] = l; X[i][j + 1] = r; }
}